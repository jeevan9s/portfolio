  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-[100rem] 3xl:max-w-[130rem] 4xl:max-w-[150rem] gap-12 3xl:gap-16 pt-16 md:grid-cols-[minmax(17rem,0.7fr)_minmax(0,1.3fr)] md:pt-24">
        ...
        >
          <p className="inter mb-3 text-sm 3xl:text-base text-[#878787] uppercase tracking-[0.18em]">{project.category ?? "Firmware"}</p>
              ...
              <Disclosure label="Links" isOpen={openPanels.links} onClick={() => setOpenPanels((panels) => ({ ...panels, links: !panels.links }))}>
                <ul className="space-y-2.5 3xl:space-y-3">
                  {projectLinks.map((link) => (
                    <li key={link.href}>
                      <a href={link.href} target="_blank" rel="noreferrer" className="inter text-sm 3xl:text-base text-[#5F5F5F] underline decoration-black/20 underline-offset-4 transition-colors hover:text-[#1E1E1E]">{link.label}</a>
                    </li>
                  ))}
                </ul>
              </Disclosure>
